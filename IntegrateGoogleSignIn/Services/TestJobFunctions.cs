...
{
    public static class TestJobFunctions
    {
                                            ...
                                            if (userData.userListing.Any())
                                            {
                                                foreach (var visitData in userData.userListing)
                                                {
                                                    if (!isFound)
                                                    {
                                                        ...
                                                        if (urlToDisplay.Contains(blueTag.searchtext) || urlTitle.Contains(blueTag.searchtext))
                                                        {
                                                            isFound = true;
        ...
        {
            var userDailyAnalytic = new List<UserDailyAnalytic>();
            var propackageDomainList = new List<string>();
            //var currentDate = new DateTime(2019, 08, 09);
            var currentDate = DateTime.UtcNow.AddDays(-1);

            CommonFunctions.SaveAuditLog("0.0: Call Scheduler DailyAnalyticUpdated() => " + currentDate.ToString("ddMMyyyy"));
            try
            {
                using (var db = new DBEntities())
                {
                    propackageDomainList = db.propackageusers.Select(m => m.domainname).Distinct().ToList();
                }
                ...
                if (propackageDomainList.Any())
                {
                    ...
                    {
                        var collection = CommonFunctions.GetDailyCountUpdated(propackageDomain, currentDate);
                        if (collection != null)
                        {
                            var wwwDomainName = propackageDomain;
                            if (!propackageDomain.Contains("www."))
                            {
                                wwwDomainName = "www." + propackageDomain;
                            }

                            userDailyAnalytic.Add(new UserDailyAnalytic
                            {
                                bluetickvisitors = collection.blueVisitors,
                                collectiondate = currentDate,
                                domainname = propackageDomain,
                                greentickvisitors = collection.greenVisitors,
                                multivisitors = collection.multiVisitors,
                                redtickvisitors = collection.redVisitors,
                                pinktickvisitors = collection.pinkVisitors,
                    ...
                    if (userDailyAnalytic.Any())
                    {
                        using (var db = new DBEntities())
                        {
                            foreach (var item in userDailyAnalytic)
                            {
                                string wwwDomainName;
                                string wwwWithoutDomainName;
                                if (item.domainname.Contains("www."))
                                {
                                    wwwDomainName = item.domainname;
                                    wwwWithoutDomainName = item.domainname.Replace("www.", string.Empty);
                                }
                                else
                                {
                                    wwwDomainName = "www." + item.domainname;
                                    wwwWithoutDomainName = item.domainname;
                                }

                                var dateWiseAvailable = db.UserDailyAnalytics.Where(m => DbFunctions.TruncateTime(m.collectiondate) == currentDate.Date
                                   && (m.domainname.Equals(wwwDomainName) || m.domainname.Equals(wwwWithoutDomainName))).ToList();

                                if (dateWiseAvailable != null && dateWiseAvailable.Any())
                                {
                                    db.UserDailyAnalytics.RemoveRange(dateWiseAvailable);
                                }
                                db.UserDailyAnalytics.Add(item);
                                db.SaveChanges();
                            }
                        }
                    }
