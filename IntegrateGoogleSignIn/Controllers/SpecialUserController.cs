    ...
    public class SpecialUserController : Controller
    {
        ...
        public ActionResult Login(string email, string password)
        {
            try
            {
                ...
                var userExists =
                    db.specialusers.FirstOrDefault(m => m.email.Equals(email) && m.password.Equals(password));
                if (userExists != null)
                {
            ...
            return RedirectToAction("Login", "SpecialUser", new { area = "" });
        }
        ...
        public ActionResult ForgotPassword(string email)
        {
            try
            {
                ...
                {
                    var emailExists = db.specialusers.FirstOrDefault(m => m.email.Equals(email));
                    if (emailExists == null)
                    {
                        TempData["SwalErrorMessage"] = "User not found in the system.";
                        return RedirectToAction("ForgotPassword", "SpecialUser");
                    }
                    else
                    {
                        ...
                        return RedirectToAction("Login", "SpecialUser");
                    }
                }
            }
            catch (Exception e)
        ...
        public ActionResult Dashboard()
        {
            ...
            var domainList = db.specialusers.Where(m => m.email.Equals(email)).ToList();
            if (!domainList.Any())
            {
                domainList = new List<specialuser>();
            }
            else
            {
                foreach (var data in domainList)
                {
                    ...
                    foreach (var domainInfo in domainCollection)
                    {
                        ...
                        var monthlyReportData = GetMonthlyVisitors(domainInfo);
